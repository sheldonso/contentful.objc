...
.PHONY: all open clean doc example example-static pod really-clean travis static-lib test kill_simulator
...
travis:
		set -x -o pipefail && xcodebuild -jobs `sysctl -n hw.ncpu` test \ 
		-workspace $(WORKSPACE) -scheme 'ContentfulDeliveryAPI' | xcpretty -c \
